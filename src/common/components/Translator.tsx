...
import ImageThumb from './ImageTumb'
import { convertFileSrc } from '@tauri-apps/api/core'
import { appCacheDir, join } from '@tauri-apps/api/path'
import { fetchSSE, getSettings } from '../utils'
import {IMessageRequest} from "@/common/engines/interfaces";
...
function InnerTranslator(props: IInnerTranslatorProps) {
    ...
    const [textAreaKey, setTextAreaKey] = useState(0); // 新增 key 状态
    ...
    const getTranslateDeps = useCallback(
        async function(text: string, action: Action, imageId: string): Promise<typeof translateDeps> {
    ...
    const checkImage = async (path: string) => {
        try {
            ...
            if (!fileExists) {
                setPreviewImage(null) // 文件不存在时清空预览
                return false
            }
            return true
        } catch (e) {
            console.error('Failed to check image:', e)
            return false
        }
    }
    async function deleteOcrTempFiles() {
        try {
            await invoke('delete_temp_ocr_files');
            console.log('OCR 临时文件删除成功');
            updateImageAndRows()
        } catch (error) {
            console.error('删除 OCR 临时文件失败:', error);
        }
    }
    const updateImageAndRows = async () => {
        try {
            const dir = await homeDir();
            const fullPath = await join(dir, 'Desktop', 'cut_copy.png');

            const imageExists = await checkImage(fullPath);

            if (imageExists) {
                setEditorRows(5);
                setPreviewImage(convertFileSrc(fullPath) + `?t=${Date.now()}`);
            ...
            console.error('Error updating image and rows:', error);
            setEditorRows(5);
            setPreviewImage(null);
        }
    };
        ...
        const executeEffect = async () => {
            const handleImageUpload = async () => {
                try {
                    ...
                    const homePath = await join(home, 'Desktop', 'cut_copy.png')

                    // 读取并上传文件
                    console.log('读取本地图片数据:', homePath)
                    const fileData = await readFile(homePath)
                    const file = new Blob([fileData.buffer], { type: 'image/png' })
                    const form = new FormData()
                    form.append('file', file, 'cut_copy.png')
                    form.append('user', 'translation-user')

                    // 上传到API
                    const apiKey = settings.difyAPIKey
                    const apiURL = settings.difyAPIURL
            ...
            setActivateAction((action) => {
                ...
                getTranslateDeps(externalOriginalText, action, imageId).then((v) => {
                    setTranslateDeps(v)
                })
                return action
            })
    ...
    const translateText = useDeepCompareCallback(
        async (selectedWord: string, signal: AbortSignal) => {
            ...
            const { text, sourceLang, targetLang, action, imageId } = translateDeps
            ...
            }:${action.imageId}:${sourceLang}:${targetLang}:${text}:${selectedWord}:${translationFlag}`
        ...
        ;(async () => {
            unlisten = await listen('tauri://file-drop', async (e: Event<string>) => {
                ...
                const executeEffect = async () => {
                    ...
                    const updatePreviewImageAndUpload = async () => {
                        const dir = await appCacheDir()
                        const fullPath = await join(dir, 'ocr_images', 'cut.png')
                        setPreviewImage(convertFileSrc(fullPath) + `?t=${Date.now()}`)
                    }
                    updatePreviewImageAndUpload()
                    const imageId = await handleImageUpload()
                    console.log('Image ID:', imageId)
                    if (activateAction) {
                        const newTranslateDeps = await getTranslateDeps(data.text, activateAction, imageId)
                                    ...
                                    <div {...getRootProps()}>
                                        ...
                                        <Textarea
                                                    ...
                                                    style: {
                                                        ...
                                                        fontFamily:
                                                            currentTranslateMode === 'explain-code'
                                                                ...
                                                                : 'inherit',
                                                        textalign: 'start',
                                                        minHeight: editableText ? 'auto' : '40px', // 无内容时较小高度
                                                    },
                                                },
                                            }}
                                            value={previewImage ? '' : editableText}
                                            size='mini'
                                            resize='vertical'
                                            rows={
                                                editorRows
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (e.target.value && previewImage) {
                                                    setPreviewImage(null);
                                                    deleteOcrTempFiles()
                                                }
                                            }}
